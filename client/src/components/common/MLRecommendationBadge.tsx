...
export const MLRecommendationBadge: React.FC<MLRecommendationBadgeProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm font-sans space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-3 bg-gradient-to-r from-blue-50/70 via-slate-50 to-indigo-50/40 border border-blue-200/80 rounded-2xl p-3 sm:p-4">
        <div>
        ...
        {isDock && onAllocate && (
          <button
            onClick={onAllocate}
            className="px-3.5 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs font-mono flex items-center space-x-1.5 transition shadow-sm cursor-pointer whitespace-nowrap shrink-0"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span className="whitespace-nowrap">Approve {targetName.split(' ')[0]}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1 font-mono text-xs">
        <div className="space-y-1.5 bg-slate-50/90 p-3 sm:p-3.5 rounded-2xl border border-slate-200/80">
          ...
          <ul className="space-y-1.5 text-[11px] text-slate-600">
            {factors && factors.length > 0 ? (
              factors.map((f, idx) => (
                <li key={idx} className="flex items-start space-x-1.5 text-emerald-700">
              ...
              <>
                <li className="flex items-start space-x-1.5 text-emerald-700">
                  <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0 text-emerald-600 mt-0.5" />
          ...
          {alternatives && alternatives.length > 0 ? (
            <div className="space-y-1.5">
              {alternatives.map((alt: any, idx) => (
                <div key={idx} className="flex items-center justify-between text-[11px] text-slate-700 gap-2">
                  <span className="font-semibold whitespace-nowrap">
                    #{idx + 2} {isDock ? `Dock ${alt.dockId}` : `Slot ${alt.slotId}`}
                  </span>
                  <span className="text-slate-500 font-bold bg-white px-2 py-0.5 rounded border border-slate-200 shadow-2xs whitespace-nowrap shrink-0">
