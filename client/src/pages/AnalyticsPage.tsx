...
export const AnalyticsPage: React.FC = () => {
  ...
  const liveDockData = useMemo(() => {
    if (!docks || docks.length === 0) {
      return [
        { name: 'D01 (Dry)', utilization: 85, type: 'DRY_VAN', status: 'OCCUPIED' },
        { name: 'D02 (Dry)', utilization: 40, type: 'DRY_VAN', status: 'AVAILABLE' },
        { name: 'D03 (Flatbed)', utilization: 90, type: 'FLATBED', status: 'OCCUPIED' },
        { name: 'D04 (Reefer)', utilization: 95, type: 'REFRIGERATED', status: 'MAINTENANCE' },
        { name: 'D05 (Reefer)', utilization: 60, type: 'REFRIGERATED', status: 'AVAILABLE' },
        { name: 'D06 (Hazmat)', utilization: 25, type: 'HAZMAT', status: 'AVAILABLE' },
      ];
    }
  ...
  const exceptionPieData = useMemo(() => {
    ...
    if (exceptions.length > 0) {
      exceptions.forEach(ex => {
        if (ex.type === 'DOCK_FAILURE') counts['Dock Failure'] += 1;
        else if (ex.type === 'LONG_WAITING' || ex.type === 'EXTENDED_PROCESSING') counts['Demurrage Dwell'] += 1;
        else if (ex.type === 'SHIPMENT_DELAY' || ex.type === 'MISSED_APPOINTMENT') counts['ETA Delay'] += 1;
        else if (ex.type === 'YARD_LOCATION_MISMATCH' || ex.type === 'YARD_CONGESTION') counts['Location Mismatch'] += 1;
        else counts['ETA Delay'] += 1;
      });
    } else {
      counts['Dock Failure'] = 1;
      counts['Demurrage Dwell'] = 2;
      counts['ETA Delay'] = 1;
      counts['Location Mismatch'] = 1;
    }

    const palette = ['#ef4444', '#f59e0b', '#2563eb', '#8b5cf6'];
  ...
  const getCellColor = (val: number, isMaint: boolean = false) => {
    if (isMaint) return 'bg-rose-100 text-rose-800 border-rose-200';
    if (val === 0) return 'bg-slate-50 text-slate-400 border-slate-100';
    if (val < 35) return 'bg-emerald-50 text-emerald-700 border-emerald-100';
    if (val < 65) return 'bg-blue-50 text-blue-700 border-blue-100';
    ...
    <div className="space-y-6 font-mono">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 font-bold text-[10px] border border-emerald-200 flex items-center space-x-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          value={`${heatmapData?.metrics?.avgTurnaroundMins || 38} min`}
        ...
        <KPICard
          ...
          value={`$${(heatmapData?.metrics?.demurrageSavedDollars || 18450).toLocaleString()}`}
        ...
        <KPICard
          ...
          value={`${kpis?.dockUtilizationPercent || 33}%`}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  heatmapFilter === 'ALL' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                All ({heatmapData?.docks?.length + (heatmapData?.yardZones?.length || 0) || 9})
              </button>
              <button
                onClick={() => setHeatmapFilter('DOCKS')}
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  heatmapFilter === 'DOCKS' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                Docks (6)
              </button>
              <button
                onClick={() => setHeatmapFilter('YARD')}
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  heatmapFilter === 'YARD' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                Yard Zones (3)
              </button>
        ...
        <div className="overflow-x-auto pb-2">
          <div className="min-w-[780px]">
            ...
            >
              <div className="text-left font-bold text-slate-700 pl-1 uppercase">Asset / Door</div>
                ...
                >
                  <div className="text-left pr-2 truncate">
                    <div className="font-bold text-xs text-slate-900 flex items-center space-x-1.5">
                      <span className={`w-1.5 h-1.5 rounded-full ${row.type === 'DOCK' ? 'bg-blue-600' : 'bg-amber-600'}`}></span>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs">
          <div className="flex items-center space-x-2">
            <span className="text-[11px] text-slate-400 font-bold uppercase">Intensity:</span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                Live Dock Door Utilization % by Capability
              </h3>
            </div>
          ...
          <div className="h-64 w-full text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={liveDockData}>
                ...
                <Tooltip
                  formatter={(value: any) => [`${value}% Utilization`, 'Door Load']}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                Hourly Yard Occupancy % vs Target Limit
              </h3>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                Average Unload Turnaround &amp; Wait Time (Mins)
              </h3>
            </div>
          ...
          <div className="h-64 w-full text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={waitTrendData}>
                ...
                <Tooltip
                  formatter={(value: any, name: any) => [`${value} mins`, name === 'turnaround' ? 'Total Turnaround' : 'Wait Time']}
                  contentStyle={{ backgroundColor: '#FFFFFF', borderColor: '#E2E8F0', borderRadius: 8, fontSize: 11 }}
                />
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                Active Exception Distribution by Category
              </h3>
            </div>
