    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-xs">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-400">
          <span className="font-mono text-[11px] font-bold text-slate-500">Live Samples:</span>
          {['TR-219', 'TR-202', 'TR-211', 'TR-203', 'SHP-1009', 'TR-106', 'SHP-1001'].map(id => (
            <button
              ...
              className="px-2.5 py-1 rounded-lg font-mono text-[11px] font-semibold bg-slate-100 hover:bg-slate-200 text-blue-700 border border-slate-200 transition cursor-pointer"
            >
              {id}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Main Info */}
            <div className="lg:col-span-2 bg-white border border-slate-200/90 rounded-2xl p-6 space-y-4 shadow-xs">
              <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                <div>
                  <div className="flex items-center space-x-3">
                    <h3 className="text-lg font-black text-slate-900 font-mono">{shipment.id}</h3>
                ...
                <div className="text-right font-mono">
                  <span className="text-[10px] text-slate-400 block font-bold">CARRIER</span>
                  <span className="text-sm font-black text-blue-700">{shipment.carrierName}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Supplier / Consignor</span>
                  <span className="text-slate-800 font-semibold">{shipment.supplier}</span>
                </div>

                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Payload Weight</span>
              ...
              {isCold && (
                <div className="p-3 rounded-xl bg-cyan-50/70 border border-cyan-200 text-xs font-mono text-cyan-950 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <span className="font-bold block">Cold-Chain Temperature Monitored: {shipment.temperatureProfile || 'REFRIGERATED_CHILL'}</span>
                      <span className="text-[10px] text-cyan-700">Target Spec: {shipment.targetTemperatureRange || '2°C to 4°C'} • Current: {shipment.currentTempCelsius !== undefined ? `${shipment.currentTempCelsius}°C` : '3.2°C'}</span>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 font-bold text-[10px] self-start sm:self-auto">
                    ✓ Thermal Lock Active
                  </span>
                </div>
              )}
            ...
            <div className="bg-white border border-slate-200/90 rounded-2xl p-6 space-y-4 shadow-xs">
              ...
              <div className="space-y-3 font-mono text-xs">
                <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  <span className="text-slate-500">Loaded on Rig:</span>
                  <span className="font-black text-blue-700">{shipment.trailerId}</span>
                </div>
                ...
                <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  <span className="text-slate-500">Yard Staging:</span>
                ...
                <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  <span className="text-slate-500">Appt Window:</span>
                  <span className="text-slate-800 font-semibold">
