      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            {isLive ? (
              <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-emerald-50 text-emerald-700 rounded-md border border-emerald-200 flex items-center space-x-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping mr-0.5" />
                <span>LIVE PHYSICAL TELEMETRY</span>
              </span>
            ) : (
              <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-blue-50 text-blue-700 rounded-md border border-blue-200 flex items-center space-x-1">
                <Calendar className="w-3 h-3 text-blue-600" />
                <span>FORECAST SHIFT: +{horizon}</span>
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 font-mono font-bold text-xs border border-amber-200 flex items-center space-x-1.5 transition cursor-pointer shadow-2xs"
            title="Simulate sub-zero cryo emergency preemption (bumps lower priority dry van to yard buffer)"
          >
            <Zap className="w-3.5 h-3.5 text-amber-600" />
            <span>Simulate Cryo Preemption</span>
          </button>

          <button
            onClick={handleTriggerDelay}
            className="px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 font-mono font-bold text-xs border border-slate-200 flex items-center space-x-1.5 transition cursor-pointer shadow-2xs"
            title="Simulate highway delay (+45m)"
          >
            <Clock className="w-3.5 h-3.5 text-blue-600" />
            <span>Simulate Delay (+45m)</span>
          </button>

          <button
            onClick={() => fetchDocks(horizon)}
            className="p-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 transition cursor-pointer shadow-2xs"
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <span className="text-base font-black font-mono text-slate-900">{dock.id}</span>
                    <StatusBadge status={dock.status} type="dock" size="sm" />
                    {isCold && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-cyan-50 text-cyan-800 border border-cyan-200 flex items-center space-x-1">
                        <Snowflake className="w-2.5 h-2.5" />
                        <span>{hasDeepFreeze ? '-20°C Cryo' : '2-4°C Chill'}</span>
                      </span>
                    )}
                  </div>
                ...
                <h3 className="text-xs font-bold text-slate-800 mt-1 line-clamp-1">{dock.name}</h3>
                ...
                <div className="mt-2 flex items-center space-x-1.5 flex-wrap gap-y-1">
                  ...
                  {dock.capabilities.map((cap: string) => (
                    <span
                      key={cap}
                      className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-slate-50 text-slate-700 border border-slate-200 font-bold"
                    >
                      {cap}
                    </span>
                  ))}
                  ...
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2.5 font-mono text-xs">
                    <div className="flex justify-between items-center text-slate-800">
                      <div className="flex items-center space-x-1.5">
                        ...
                        <span className="font-black text-blue-700">{dock.currentTrailerId}</span>
                      </div>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-white text-slate-700 border-slate-200 shadow-2xs">
                        ⏱ Free in: {freeIn}m
                      </span>
                    </div>
                  ...
                  <div className="p-3 bg-blue-50/40 rounded-xl border border-blue-200/80 space-y-2 font-mono text-xs">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center space-x-1.5">
                        ...
                        <span className="font-black text-slate-900 text-xs">{dock.currentTrailerId}</span>
                      </div>
                      <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] py-1 border-y border-blue-200/60 text-slate-700">
                      <div>
                        <span className="text-[9px] text-slate-400 block uppercase">Scheduled Window</span>
                        <span className="font-bold text-blue-950">{dock.scheduledWindowStart || '15:00'} – {dock.scheduledWindowEnd || '15:45'}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 block uppercase">Planned Duration</span>
                        <span className="font-bold text-slate-800">~{dock.unloadingDurationMinutes || 45} mins</span>
                      </div>
                    </div>

                    <div className="flex justify-between items-center text-[10px] text-slate-500">
                      <span>Carrier / Manifest:</span>
                      <span className="font-bold text-slate-700">{dock.currentShipment?.supplier || dock.currentTrailer?.carrierName || 'Scheduled Freight'}</span>
                    </div>
                  </div>
                )
              ...
              {dock.nextQueuedTrailerId && (
                <div className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono space-y-1">
                  ...
                  <div className="flex items-center justify-between text-slate-800 text-[11px]">
                    <span className="font-black text-slate-900">{dock.nextQueuedTrailerId}</span>
              ...
              <div className="pt-2 border-t border-slate-200 flex items-center justify-between gap-2 text-xs">
                <button
                  ...
                  className="px-2.5 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-[11px] font-mono border border-slate-200 font-bold flex items-center space-x-1.5 cursor-pointer transition"
                ...
                <div className="flex space-x-1">
                  {dock.status !== 'AVAILABLE' && (
                    <button
                      ...
                      className="px-2 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-[10px] font-mono border border-emerald-200 font-bold cursor-pointer transition"
                    >
                      Available
                    </button>
                  )}
                  {dock.status !== 'BLOCKED' && (
                    <button
                      ...
                      className="px-2 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg text-[10px] font-mono border border-rose-200 font-bold cursor-pointer transition"
                    >
                      Block
                    </button>
                  )}
                  {dock.status !== 'MAINTENANCE' && (
                    <button
                      ...
                      className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-lg text-[10px] font-mono border border-amber-200 font-bold cursor-pointer transition"
                    >
                      Maint.
                    </button>
                  )}
